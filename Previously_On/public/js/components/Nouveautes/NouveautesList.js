...
class NouveautesList extends React.Component {
  ...
  componentDidMount() {
    const that = this;
    // fetch series from betaseries;
    fetch(
      "https://api.betaseries.com/shows/list?key=0ba964b97be8&order=popularity&limit=200"
    )
  ...
  render() {
    ...
    if (filter === "all") {
      ...
      for (let i = 0; i < this.state.returnSeriesFromAPI.length; i++) {
        if (j < 30) {
            let genres = this.state.returnSeriesFromAPI[i].genres.map(genre =>
